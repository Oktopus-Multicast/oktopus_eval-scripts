from . import LINE_STYLES, LEGEND_FONT_SIZE, flatui
import os
...
def set_legend(top=True, ncol=1, font_size=LEGEND_FONT_SIZE, bbox=None):
    if top:
        if ncol == 2:
            if not bbox:
...
def save_fig(full_path, default_format='eps'):
    if full_path:
        ...
        # plt.savefig(full_path, format=file_extension, dpi=1200)
